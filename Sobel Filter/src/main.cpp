...
void sobel_filter(Mat *grayscale_img, Mat *sobel_img, int start = 0, int step = 1) {
    ...
    uint16x8_t Gx_row_vect;
    uint16x8_t Gx_col_vect;
//    uint16x8_t Gy_row_vect;
//    uint16x8_t Gy_col_vect;

    // Loop through the rows and cols of the image and apply the sobel filter
    for (int row = start; row < numRows - 2; row += step) {
        for (int col = 0; col < numCols - 2; col++) {

            // index = ([nunCols * (row + x) + (col + y)] << z)
            // Convolution = img[index] + img[index] + img[index] - img[index] - img[index] - img[index]
            ...
            uint16x8_t V_row = vdupq_n_u16(row);
            uint16x8_t V_col = vdupq_n_u16(col);

            // Convolve Gx
            Gx_row_vect = {2, 1, 0, 2, 1, 0};
            Gx_col_vect = {2, 2, 2, 0, 0, 0};

            vaddq_u16(Gx_row_vect, V_row);      // Gx_row = (row + Gx[i])
            vaddq_u16(Gx_col_vect, V_col);      // Gx_col = (col + Gx[j])

            vmulq_n_u16(Gx_row_vect, numCols);  // Gx_row *= numCols
            vaddq_u16(Gx_row_vect, Gx_col_vect);// Gx_row += Gx_col

            Gx =
//                    (grayscale_data[vgetq_lane_u16(Gx_row_vect, 0)]) +
//                    (grayscale_data[vgetq_lane_u16(Gx_row_vect, 1)] << 1) +
//                    (grayscale_data[vgetq_lane_u16(Gx_row_vect, 2)]) -
//                    (grayscale_data[vgetq_lane_u16(Gx_row_vect, 3)]) -
//                    (grayscale_data[vgetq_lane_u16(Gx_row_vect, 4)] << 1) -
//                    (grayscale_data[vgetq_lane_u16(Gx_row_vect, 5)]);
...
//            Gy_row_vect = {0, 0, 0, 2, 2, 2};
//            Gy_col_vect = {0, 1, 2, 0, 1, 2};
            ...
            // Overflow check
            if (G > 255) {
                sobel_data[(sobel_img->cols * (row) + (col))] = 255;
            }
