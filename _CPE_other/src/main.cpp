 ...
 *
 * Revisions:
 *      - (1.2) Threads are redone to be created once and not destroyed until
 *        completion.
 *      - (1.1) Original
 *
 *****************************************************************************/
...
int main(int argc, char const *argv[])
{
    ...
    if (ifile) {
        ...
        auto numRows = (uint16_t) usr_vid.get(4);
        auto numCols = (uint16_t) usr_vid.get(3);

        // Define Mat objects used
        Mat frameOrig(numRows, numCols, CV_8UC3);
        Mat frameGrey(numRows, numCols, CV_8UC1);
        Mat frameSobel(numRows - 2, numCols - 2, CV_8UC1);

        // Create threads
        pthread_t threads_grey[numThreads];
        pthread_t threads_sobel[numThreads];
        ...
        struct thread_sobel_data td_grey2sobel[numThreads];
        for (uint8_t thread = 0; thread < numThreads; ++thread) {
            ...
            td_orig2grey[thread].endPos = (thread + 1)*numCols*(numRows/numThreads);
            ...
            td_grey2sobel[thread].inputFrame = &frameGrey;
            td_grey2sobel[thread].outputFrame = &frameSobel;
            td_grey2sobel[thread].startRow = thread*(numRows/numThreads) - 2;
            td_grey2sobel[thread].endRow = (thread + 1)*(numRows/numThreads) - 1;
            td_grey2sobel[thread].numRows = numRows;
            td_grey2sobel[thread].numCols = numCols;
            td_grey2sobel[thread].thread_id = thread;

            pthread_create(&threads_sobel[thread], nullptr, &sobelFilter, (void*) &td_grey2sobel[thread]);
        }
 ...
 *              Converts a 3 channel RGB image to a 1 channel greyscale image
 *              using ITU-R(BT.709) recommended algorithm.
 *
 *              Grey = 0.2126R + 0.7152G + 0.0722B
 *
 ...
 *****************************************************************************/
void* toGreyscale(void* threadArgs) {
    auto *frames = (struct thread_grey_data*) threadArgs;
    ...
    while (!done_flag) {
        for (uint32_t pos = frames->startPos; pos < frames->endPos; ++pos) {
            greyImgData[pos] = (uchar)(origImgData[3*pos + 0] * B_CONST) +
 ...
 *****************************************************************************/
void* sobelFilter(void* threadArgs) {
    auto *frames = (struct thread_sobel_data*) threadArgs;
    int gx, gy, G;
